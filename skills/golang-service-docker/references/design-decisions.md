...
This document explains the operational choices encoded by `service-docker-scaffold`. Read the relevant section before changing a template or bypassing the renderer.
...
The renderer owns files containing the `Generated by service-docker-scaffold` signature. It can update those files on subsequent runs but refuses to overwrite unmanaged files unless the caller supplies `--force` after reviewing the conflict.
