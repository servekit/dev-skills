...
die() {
  printf 'golang-service-docker: %s\n' "$*" >&2
  exit 1
}
...
if [[ "$scaffold_build_mode" == source ]]; then
  ...
  if [[ ! -f "${scaffold_target_abs}/bin/${scaffold_binary_name}" ]]; then
    printf 'golang-service-docker: warning: bin/%s is absent; docker-build will reject it until a static Linux ELF is provided\n' "$scaffold_binary_name" >&2
  fi
fi
...
render_template() {
  ...
  local scaffold_tmp
  scaffold_tmp="$(mktemp "${scaffold_target_abs}/.golang-service-docker.XXXXXX")"
...
managed_or_absent() {
  ...
  grep -q 'Generated by golang-service-docker' "$scaffold_path" && return 0
...
if [[ ! -f "${scaffold_target_abs}/.env.example" ]]; then
  ...
  printf 'golang-service-docker: read existing .env.example for compose defaults (not regenerated)\n' >&2
fi

scaffold_make_block="$(mktemp "${scaffold_target_abs}/.golang-service-docker.make.XXXXXX")"
...
scaffold_make_base="$(mktemp "${scaffold_target_abs}/.golang-service-docker.make-base.XXXXXX")"

if [[ -f "$scaffold_makefile" ]]; then
  scaffold_begin_count="$(grep -c '^# BEGIN golang-service-docker$' "$scaffold_makefile" || true)"
  scaffold_end_count="$(grep -c '^# END golang-service-docker$' "$scaffold_makefile" || true)"
  [[ "$scaffold_begin_count" == "$scaffold_end_count" ]] || die "Makefile has an incomplete golang-service-docker block"
  [[ "$scaffold_begin_count" == 0 || "$scaffold_begin_count" == 1 ]] || die "Makefile has duplicate golang-service-docker blocks"
  awk '
    /^# BEGIN golang-service-docker$/ { skip = 1; next }
    /^# END golang-service-docker$/ { skip = 0; next }
    !skip { print }
...
scaffold_make_combined="$(mktemp "${scaffold_target_abs}/.golang-service-docker.make-final.XXXXXX")"
...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
  ...
  printf 'golang-service-docker: warning: Docker Compose unavailable; skipped compose validation\n' >&2
fi
