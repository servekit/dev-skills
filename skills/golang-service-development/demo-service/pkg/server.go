// Package pkg is the public surface of demo-service. It wires internal pieces
// into a runnable gRPC/HTTP server, a gRPC client, and an in-process module
// entry point. Downstream services that depend on demo-service should import
...
// Server wraps a gRPC + HTTP gateway server for the demo service.
//
...
//     rules declared in demo.proto
//
// The HTTP gateway auto-registers via demov1.RegisterDemoServiceHandlerFromEndpoint
...
func NewServer(cfg *config.Config, opts ...ServerOption) (*Server, error) {
	...
	grpcSrv := grpcx.New(
		...
		func(gs *grpc.Server) {
			demov1.RegisterDemoServiceServer(gs, hdl)
		},
		demov1.RegisterDemoServiceHandlerFromEndpoint,
