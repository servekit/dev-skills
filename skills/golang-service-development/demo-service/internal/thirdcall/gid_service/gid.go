// Package gid_service adapts gid-service to demo-service's internal needs.
//
// The GIDService interface is internal: external callers inject the raw
// *gidservice.Handler via option.WithGIDHandler; this package wraps it into
// the clean NextID shape that demo-service's business code consumes. Building
// the raw Handler (or threading one in from a parent process) happens in the
// service root (internal/service/helper.go resolveGID); this package only
// wraps.
package gid_service
